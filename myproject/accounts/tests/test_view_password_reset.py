...
class SuccessfulPasswordResetTests(TestCase):
    def setUp(self):
        ...
        User.objects.create(username='john', email=email, password='12345abcdef')
        url = reverse('password_reset')
        self.response = self.client.post(url, {'email': email})

    def test_redirection(self):
        '''
        Valid form submission should redirect the user to password_reset_done view
        '''
...
class InvalidPasswordResetTests(TestCase):
    def setUp(self):
        url = reverse('password_reset')
        self.response = self.client.post(url, {'email': 'nonexistent@gmail.com'})

    def test_redirection(self):
        '''
        Invalid emails in database should redirect user to password_reset_done view
        '''
        url = reverse('password_reset_done')
        self.response = self.client.get(url)
...
class PasswordResetConfirmTests(TestCase):
    def setUp(self):
        user = User.objects.create_user(username='john', email='john@gmail.com', password='1234abcd')
        '''
    ...
    def test_form_inputs(self):
        '''
        The view should contain two inputs: csrf and two password fields
        '''
